/* ════════════════════════════════════════════════════════════════
   PW STUDY HUB — Telegram Mini App (Claude Editorial Edition)
...
const CONFIG = {
  ...
  SERVER_URL: 'https://pw-bot-nu5b.onrender.com',
...
async function checkInAppForceSub(manualVerify = false) {
  ...
  try {
    ...
    if (data.subscribed) {
      ...
      if (manualVerify) {
        haptic('success');
        showToast('✅ Verification Successful! Welcome to RangeXCoder.');
      }
      return true;
    }
...
function updateBreadcrumbs(view) {
  ...
  if (view === 'guru') {
    ...
    trail.innerHTML = `<span class="crumb-link" onclick="navigate('platforms')">🏛️ Platforms</span> <span class="crumb-sep">›</span> <span class="crumb-current">🤖 RangeXCoder Guru</span>`;
    return;
  }
...
async function fetchBatchContent(batchId, subjectSlug, topicSlug, type = 'Videos', page = 1) {
  const safeTag = encodeURIComponent(topicSlug || '');
  const safeBatch = encodeURIComponent(batchId || '');
  const safeSubject = encodeURIComponent(subjectSlug || '');
  const providerParam = state.provider ? `&provider=${state.provider}` : '';
  
  // Normalize contentType for pimaxer API:
  ...
  try {
    const res = await serverGet(
      `/api/batch/${safeBatch}/subject/${safeSubject}/content?tag=${safeTag}&type=${type}&page=${page}${providerParam}`
    );
    if (res && res.data && Array.isArray(res.data)) return res;
    if (res && Array.isArray(res)) return { data: res };
    if (res && res.data) return res;
  } catch (err) {
    console.warn(`[fetchBatchContent] Server proxy failed (${type}):`, err.message);
  }

  // 2. Direct Fallback via a.pimaxer.in (Only for PW)
  if (state.provider === 'pw') {
    try {
      const directUrl = `${CONFIG.CONTENT_API}/batches/${safeBatch}/subject/${safeSubject}/content?page=${page}&contentType=${contentType}&tag=${safeTag}`;
      const res = await fetch(directUrl, {
        headers: CONFIG.CONTENT_HEADERS,
      });
      if (res.ok) return await res.json();
    } catch (directErr) {
      console.warn(`[fetchBatchContent] Direct fetch failed (${type}):`, directErr.message);
    }
  }
...
function openBatch(batchRef) {
  ...
  document.getElementById('batch-hero-class').textContent = b.class ? `Class ${b.class}` : (PROVIDERS[state.provider]?.name || 'RangeXCoder');
...
function openTopic(topicRef) {
  ...
  // Pimaxer uses subject._id (not slug) as the subjectSlug path param
  const subjectSlug = state.subject._id;
  // Pimaxer uses topic._id (not slug) as the tag query param
  const topicSlug = t._id;
...
async function loadContent(type, reset = false) {
  ...
  // Pimaxer uses subject._id (not slug) as the subjectSlug path param
  const subjectSlug = state.subject._id;
  const topicSlug = state.topic._id;
...
function renderNotesItem(item, type) {
  ...
  return hws.map((hw, idx) => {
    ...
    state.loadedPayloads[payloadKey] = {
      ...
      subjectId:  item.subjectId || state.subject?._id || state.subject?.id,
...
// ═══════════════════════════════════════════════════════════════
// ACTION SELECTION SHEET & RANGEXCODER VIDEO PLAYER
// ═══════════════════════════════════════════════════════════════
...
// ─── RangeXCoder Modern In-App Player ─────────────────────────
...
function launchRangeXPlayer() {
  ...
  console.log('[RangeXCoder Player] Loading stream:', cleanUrl);
...
// ═══════════════════════════════════════════════════════════════
const THEME_STORAGE_KEY = 'rangexcoder_theme';
...
function initPWA() {
  ...
  window.addEventListener('appinstalled', () => {
    ...
    haptic('success');
    showToast('🎉 RangeXCoder App Installed Successfully!');
  });
...
function openInChromeBrowser() {
  ...
  const targetDomain = isNetlify ? window.location.origin : 'https://rangexcoder.netlify.app';
  ...
  if (deferredPwaPrompt) {
    try {
      ...
      if (choice.outcome === 'accepted') {
        showToast('⏳ Installing RangeXCoder App...');
      }
      deferredPwaPrompt = null;
      closeInstallModal();
    } catch (e) {
...
let userReferralData = {
  ...
  referrals: 0,
  target: 3,
  invite_link: 'https://t.me/RangeXCoder_studybot'
};
  ...
  try {
    ...
    if (data && data.success) {
      userReferralData = {
        ...
        invite_link: data.invite_link || `https://t.me/RangeXCoder_studybot?start=ref_${uid}`
      };
      renderVipStatusUI();
    }
  } catch (e) {
...
function copyReferralLink() {
  const link = userReferralData.invite_link || (state.chatId ? `https://t.me/RangeXCoder_studybot?start=ref_${state.chatId}` : 'https://t.me/RangeXCoder_studybot');
...
function shareReferralTelegram() {
  const link = userReferralData.invite_link || (state.chatId ? `https://t.me/RangeXCoder_studybot?start=ref_${state.chatId}` : 'https://t.me/RangeXCoder_studybot');
  const text = `🔥 Join RangeXCoder Study Hub for free Physics Wallah, NextToppers & JEE/NEET Batches, full lecture video streaming, notes & DPPs!`;
...
// ═══════════════════════════════════════════════════════════════
// FEATURE 4: RANGEXCODER GURU AI DOUBT SOLVER ENGINE
// ═══════════════════════════════════════════════════════════════
...
async function askGuruAI() {
  ...
  const timer2 = setTimeout(() => {
    if (loadingText) loadingText.textContent = '✍️ Formulating step-by-step verified solution...';
