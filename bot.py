...
BOT_TOKEN = '8822004090:AAGGfKxQKTe_VypYfn6nD03miEqOWsMjR5k'
BOT_USERNAME = 'RangeXCoder_studybot'
TG_API_BASE = f'https://api.telegram.org/bot{BOT_TOKEN}'

WEBAPP_URL = os.environ.get('WEBAPP_URL') or 'https://rangexcoder.netlify.app/'
if not WEBAPP_URL.endswith('/'):
    WEBAPP_URL += '/'

logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')
logger = logging.getLogger('RangeXCoderBot')
...
DEFAULT_SETTINGS = {
    ...
    'forcesub': False,
    'channel_link': 'https://t.me/+2wmMyRob0Os0MTRl',
    'channel_id': -1002711219748,
    'group_link': 'https://t.me/rangexcoder_chat',
...
PERMANENT_OWNERS = {
    7974991150: {'name': 'Amber Kasaudhan', 'username': 'badboy_2999'},
    8508166126: {'name': 'BAD BOY', 'username': 'badboy_295'}
}
...
USER_COMMANDS = [
    {'command': 'start', 'description': 'Launch RangeXCoder'},
...
async def verify_user_sub(user_id: int) -> dict:
    ...
    group_target = settings_data.get('group_id') or '@rangexcoder_chat'
...
async def send_forcesub_restriction(chat_id: int, sub_info: dict, message_id_to_edit: int = None):
    channel_url = settings_data.get('channel_link', 'https://t.me/+2wmMyRob0Os0MTRl')
    group_url = settings_data.get('group_link', 'https://t.me/rangexcoder_chat')
    ...
    if channel_missing and group_missing:
        desc = (
            ...
            "To use **RangeXCoder Study Bot** and launch the Mini App, you must join our official community:\n\n"
...
async def send_verified_welcome(chat_id: int, first_name: str, app_url: str):
    ...
    welcome_text = (
        f"⚡ **HEY {first_name.upper()}! WELCOME TO RANGEXCODER** ⚡\n\n"
        ...
        f"👨‍💻 **Developer:** Amber Kasaudhan | [Badboy](https://t.me/badboy_295)\n\n"
        f"✨ **Features:**\n"
    ...
    banner_url = f"{app_url}banner.jpg"
...
async def setup_bot_profile(app_url: str):
    ...
    await tg_call('setMyShortDescription', {
        'short_description': 'RangeXCoder - Free PW, NextTopper & MissionJEET batches.'
    })
                        ...
                        if curr_count >= 3:
                            asyncio.create_task(tg_call('sendMessage', {
                                ...
                                'text': (
                                    ...
                                    "👑 You are now an official **VIP Student** on RangeXCoder Study Hub!\n"
                                    "Enjoy priority fast streaming and your glowing VIP Crown badge in the app.\n\n"
                                    "⚡ *Powered by @badboy_295*"
                                ),
                                'parse_mode': 'Markdown'
                            }))
                        else:
                            asyncio.create_task(tg_call('sendMessage', {
                                ...
                                'text': (
                                    ...
                                    "━━━━━━━━━━━━━━━━━━━━━━\n"
                                    f"Your friend **{first_name}** just joined RangeXCoder!\n"
...
async def handle_callback_query(cq: dict):
    ...
    if data == 'check_forcesub':
        ...
        await tg_call('answerCallbackQuery', {
            'callback_query_id': cq_id,
            'text': "✅ Verification Successful! Welcome to RangeXCoder."
        })
        msg_id = cq.get('message', {}).get('message_id')
        if msg_id and chat_id:
            await tg_call('deleteMessage', {'chat_id': chat_id, 'message_id': msg_id})
        ...
        try:
            await tg_call('sendMessage', {
                ...
                'text': (
                    ...
                    "You now have access to the RangeXCoder admin command suite.\n\n"
...
async def handle_admincommandenable(msg: dict):
    ...
    admin_suite_guide = (
        ...
        f"Congratulations {first_name}, you have claimed permanent Super Admin ownership of RangeXCoder Bot.\n\n"
...
async def handle_forcesub(msg: dict):
    ...
    if len(parts) > 1:
        ...
        if arg in ('on', 'enable', '1', 'true'):
            ...
            g_target = settings_data.get('group_id') or '@rangexcoder_chat'
            await tg_call('sendMessage', {
                'chat_id': chat_id,
                'text': (
...
async def handle_ban(msg: dict):
    ...
    await tg_call('sendMessage', {
        'chat_id': chat_id,
        'text': f"🔨 **User `{target_id}` has been BANNED from RangeXCoder.**",
        'parse_mode': 'Markdown'
    })
...
async def handle_exportusers(msg: dict):
    ...
    if not os.path.exists(USERS_FILE):
        ...
        return
    caption = f"📁 **RangeXCoder Users Database Backup**\nTotal records: {len(users_data)}"
...
async def handle_setchannelid(msg: dict):
    ...
    try:
        ...
        adm_str = "✅ Bot is Admin in this channel!" if is_adm else "⚠️ Warning: Bot is not an Admin yet. Please add @RangeXCoder_studybot as admin in channel."
        await tg_call('sendMessage', {
            'chat_id': chat_id,
...
async def handle_checkadmin(msg: dict):
    ...
    c_id = settings_data.get('channel_id')
    g_target = settings_data.get('group_id') or '@rangexcoder_chat'
    c_link = settings_data.get('channel_link', 'https://t.me/+2wmMyRob0Os0MTRl')
    ...
    if not c_id:
        ...
        c_status = "❌ **Bot is NOT Admin**\n👉 *Add @RangeXCoder_studybot as Admin in Channel Settings*"

    if g_admin:
        g_status = "✅ **Bot is Admin!** (Members can be verified)"
    else:
        g_status = "❌ **Bot is NOT Admin**\n👉 *Add @RangeXCoder_studybot as Admin in Group Settings*"
...
async def handle_admin_panel(msg: dict):
    ...
    g_target = settings_data.get('group_id') or settings_data.get('group_link', '@rangexcoder_chat')
    ...
    panel_text = (
        "👑 **RANGEXCODER ADMIN CONTROL PANEL** 👑\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def handle_help(msg: dict):
    ...
    if is_admin(from_id):
        help_text = (
            "📖 **RANGEXCODER ADMIN COMMAND MANUAL** 📖\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def handle_invite(msg: dict):
    ...
    share_text = "🔥 Join RangeXCoder Study Hub for free Physics Wallah, NextToppers & JEE/NEET Batches, full lecture video streaming, notes & DPPs!"
    share_url = f"https://t.me/share/url?url={urllib.parse.quote(invite_link)}&text={urllib.parse.quote(share_text)}"
    
    text = (
        f"🎁 **RangeXCoder Referral & VIP Program** 👑\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Hey **{first_name}**!\n\n"
        f"Invite **{target} friends** to join RangeXCoder Bot and automatically unlock permanent **VIP Student Status**!\n\n"
...
async def handle_guru_telegram(msg: dict):
    ...
    if not question and not photo_b64:
        await tg_call('sendMessage', {
            'chat_id': chat_id,
            'text': (
                "🤖 **RangeXCoder Guru AI Doubt Solver** ⚡\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                ...
                "• Or open the **🤖 RangeXCoder Guru** tab directly in the Mini App!\n\n"
                "⚡ *Powered by @badboy_295*"
            ),
            'parse_mode': 'Markdown'
        })
                ...
                for upd in res['result']:
                    ...
                    if fwd_chat and is_admin(from_id):
                        ...
                        if c_id:
                            ...
                            status_emoji = "✅ Verified: Bot is Admin!" if bot_admin else "⚠️ Warning: Bot is NOT an Admin yet in this channel. Please promote @RangeXCoder_studybot to Admin."
                    ...
                    if cmd in ADMIN_COMMANDS_SET:
                        if not is_admin(from_id):
                            await tg_call('sendMessage', {
                                ...
                                'text': (
                                    ...
                                    "You are not an authorized administrator for RangeXCoder Bot.\n"
                        ...
                        if cmd in ('/admin', '/adminhelp'):
                            ...
                            await tg_call('sendMessage', {
                                'chat_id': msg['chat']['id'],
                                'text': (
                                    "📁 **RangeXCoder Video & PDF Vault Stats** ⚡\n"
                                    "━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def execute_bot_send(data: dict) -> tuple:
    ...
    # Use RangeXCoder branded banner for all AS Multiverse providers or if AS Multiverse / external watermark
    if provider in AS_MULTIVERSE_PROVIDERS or not image or 'asmultiverse' in str(image).lower() or 'ibb.co' in str(image).lower():
        image = f"{WEBAPP_URL}banner.jpg"
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                for ch in chapters:
                    ...
                    c_id = str(ch.get('_id'))
                    is_pdf = (c_type == 'PDF')
                    banner_img = f"{WEBAPP_URL}banner.jpg"
...
async def check_sub_data(user_id: int = None) -> dict:
    ...
    base_response = {
        ...
        'channel_link': settings_data.get('channel_link', 'https://t.me/+2wmMyRob0Os0MTRl'),
        'group_link': settings_data.get('group_link', 'https://t.me/rangexcoder_chat')
    }
...
async def main():
    ...
    await site.start()
    logger.info(f'RangeXCoder Server running on port {port} | WebApp: {app_url}')
